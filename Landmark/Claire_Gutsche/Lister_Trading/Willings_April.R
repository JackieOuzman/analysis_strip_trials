...
#file://Fssa2-adl/clw-share1/Microlab/value_soil_testing_prj/Yield_data/Landmark/Claire_Gutsche_6/Lister_Trading_1/Willings/processing/Willings_Yld_Seg_ID_zone.csv

"W:\value_soil_testing_prj\Yield_data\Landmark\Claire_Gutsche_6\Lister_Trading_1\Willings_Willows\output\Willings_Yld_SegID_zone_edit.csv"

#As it appears in the database
Organisation_db	  = "Landmark"
Contact_db = "Claire_Gutsche_6"
Farmer_db  =  "Lister_Trading_1"
Paddock_tested_db  =	"Willings_Willows"
#Zone_db            = 
data_file       = "Willings_Yld_SegID_zone_edit.csv"
...
seg_ID <- seg_ID %>% 
     mutate(Rates = case_when(Rates == 190 ~ 180,
            TRUE ~ Rates ))
...
test <- seg_ID %>% group_by(SegmentID, Rates) %>% tally()
rate1
Grower_rate
seg_ID %>% group_by(Rates) %>% tally()
...
segments <- ggplot(seg_ID_t_test_summary, aes(SegmentID , Yld, group = Rate_as_factor))+
  ...
  labs(x= "Distance along the strip (meters)",
       ...
       caption = paste("No growers yield data captured should be 90 - 0 used for P test", 
                       Additional_fert_label, 
                       sep="\n"))+
...
zone_av_1_rate3vsGR_res_sig
 ...
 zone_1 <- ggplot( zone_av_1, aes(Rate_as_factor, Yld))+
   ...
   labs(x = Fert_legend_name,
        y= "Yield t/ha",
        title = zone1,
        caption = "Below table reports mean values and significant differences compared to 0")+
