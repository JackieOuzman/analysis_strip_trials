...
#"file://Fssa2-adl/clw-share1/Microlab/value_soil_testing_prj/Yield_data/Landmark/Hamish_Verco_7/Ballinger_1/Reschkes_North/output/ReschkesNorth_Yld_SegID_Zone.csv"
"W:\value_soil_testing_prj\Yield_data\Landmark\Hamish_Verco_7\Ballinger_1\Reschkes_North\output\ReschkesNorth_Yld_SegID_Zone.csv"

#As it appears in the database
Organisation_db	  = "Landmark"
Contact_db = "Hamish_Verco_7"
Farmer_db  =  "Ballinger_1"
Paddock_tested_db  =	"Reschkes_North"
#Zone_db            = 
data_file       = "ReschkesNorth_Yld_SegID_Zone.csv"
...
# change any non numbric values 
 # seg_ID <- seg_ID %>% 
 #   mutate(Rates = case_when(Rates == "100A" ~ "100",
 #          TRUE ~ Rates ))
 # 
 # seg_ID <- filter(seg_ID,
 #                  Rates != "100B")
...
#seg_ID <- dplyr::filter(seg_ID, SegmentID != 430)
...
segments <- ggplot(seg_ID_t_test_summary, aes(SegmentID , Yld, group = Rate_as_factor))+
  ...
  annotate("rect", xmin = zone1_min, xmax = zone1_max, ymin = 0, ymax = 7, #Zone 1
           alpha = .2) +
  annotate("text", x = zone1_range, y= 1,label = zone1)+
  
  annotate("rect", xmin =zone2_min , xmax = zone2_max, ymin = 0, ymax = 7, #zone 2
           alpha = .2)+
...
list_rates
seg_ID_t_test_summary
#seg_ID_t_test_summary$Rates <- as.numeric(seg_ID_t_test_summary$Rates)
...
zone_av_1_rate3vsGR_res_sig
 ...
 zone_1 <- ggplot( zone_av_1, aes(Rate_as_factor, Yld))+
   ...
   annotate("text", x = 3, y= 4, size = 3,label = "box plot = 25%, 50%, 75%, dashed line = mean")
 zone_1
 ...
 mean_zone_av_1 <-  group_by(zone_av_1, Rates) %>% 
   summarise(mean(Yld))
 mean_zone_av_1$Rates <- as.numeric(mean_zone_av_1$Rates)
 ...
 zone_av_1$Rates <- as.numeric(zone_av_1$Rates)
 ...
 mean_zone_av_2 <-  group_by(zone_av_2, Rates) %>% 
   summarise(mean(Yld))
 mean_zone_av_2$Rates <- as.numeric(mean_zone_av_2$Rates)
