...
#file://Fssa2-adl/clw-share1/Microlab/value_soil_testing_prj/Yield_data/Landmark/Heath_Griffith_5/Campbell_2/Brennans/output/Brennans_Yield_SegID_Zone.csv
"W:\value_soil_testing_prj\Yield_data\Landmark\Heath_Griffith_5\Campbell_2\Brennans\output\Brennans_Yld_SegID_Zones.csv"

#As it appears in the database
Organisation_db	  = "Landmark"
Contact_db = "Heath_Griffith_5"
Farmer_db  =  "Campbell_2"
Paddock_tested_db  =	"Brennans"
#Zone_db            = 
data_file       = "Brennans_Yld_SegID_Zones.csv"
...
Fert_legend_name <- "Fertliser Rates"
...
seg_ID <-seg_ID %>% dplyr::select(-Rate_Units)
seg_ID <-seg_ID %>% 
  rename_at(vars(contains("ate")), ~"Rates") %>% 
  rename_at(vars(contains("one")), ~"Zone") %>% 
  rename_at(vars(contains("Yld_Mass_D")), ~"Yld") 
...
seg_ID <- dplyr::filter(seg_ID, SegmentID != 20)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 2130)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 1690)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 1700)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 2150)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 2140)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 70)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 80)
...
### should get this from harms database
 ...
 #bring in the excel sheet as a r object - not that Harm data now has a password prtotection so I need to save as csv first
 
 
 
 
 #"\\FSSA2-ADL\clw-share1\Microlab\value_soil_testing_prj\data_base\N&P 2019 data for analysis Vic 16 April2020.xlsx"
# harm_database <- read_excel(paste0(
#                             database_name_of_path,"/", "N&P 2019 data for analysis Vic 16 April2020.xlsx"),
#                             sheet = "2019 full data", range = cell_cols("A:O"))
 harm_database <- read_csv(paste0(database_name_of_path,"/", 
                                    "N&P 2019 data for analysis Vic 16 April2020.csv"))
