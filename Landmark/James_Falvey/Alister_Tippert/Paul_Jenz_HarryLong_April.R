...
#"W:\value_soil_testing_prj\Yield_data\Landmark\Alister_Tippert_1\PaulJenz_3\Harrys_Long_2\output\HarrysLong2_Yld_SegID_Zones.csv"


#As it appears in the database
Organisation_db	  = "Landmark"
Contact_db = "Alister_Tippert_1"
Farmer_db  =  "PaulJenz_3"
Paddock_tested_db  =	"Harrys_Long_2"
#Zone_db            = 
data_file       = "HarrysLong2_Yld_SegID_Zones.csv"
...
seg_ID <-seg_ID %>% dplyr::select(- Rate_Units, - Rate)
...
segments <- ggplot(seg_ID_t_test_summary, aes(SegmentID , Yld, group = Rate_as_factor))+
  ...
  #annotate("text", x = 250, y= 3,label = "Missing some data in high zone")
...
harm_database  <- read_csv(paste0(database_name_of_path,"/","N&P 2019 data for analysis Vic 16 April2020.csv"))
 ...
 site <- filter(harm_database,
                Paddock_tested == Paddock_tested_db) %>% 
                #Paddock_tested == "School_clay_loam") %>% 
                #Paddock_tested == Paddock_tested_db &
                #  Farmer == Farmer_db) %>% 
   dplyr::select(5, 6: 11)
 site
