...
#"\\FSSA2-ADL\clw-share1\Microlab\value_soil_testing_prj\Yield_data\Landmark\James_Falvey_2\Lehmann_3\Swamp\output\Swamp_Yld_SegID_Zone.csv"
#\\Fssa2-adl\clw-share1\Microlab\value_soil_testing_prj\Yield_data\Landmark\James_Falvey_2\Frankel_2\Front_1\output
...
Farmer_db  =  "Frankel_2"
Paddock_tested_db  =	"Front_1"
#Zone_db            = 
data_file       = "Front1_Yld_SegID_Zone.csv"
...
seg_ID <-  dplyr::select(seg_ID, - Rate_Units)
...
segments <- ggplot(seg_ID_t_test_summary, aes(SegmentID , Yld, group = Rate_as_factor))+
  ...
  annotate("rect", xmin = zone1_min, xmax = zone1_max, ymin = 0, ymax = 4, #Zone 1
           alpha = .2) +
  annotate("text", x = zone1_range, y= 3,label = zone1)+
...
annotate("text", x = 100, y= 3,label = "Missing data")
