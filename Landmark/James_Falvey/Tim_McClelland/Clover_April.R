...
#file://Fssa2-adl/clw-share1/Microlab/value_soil_testing_prj/Yield_data/Landmark/James_Falvey_2/Tim_McClelland_4/Clover/output/Clover_Yld_SegID_Zone.csv

#As it appears in the database
Organisation_db	  = "Landmark"
Contact_db = "James_Falvey_2"
Farmer_db  =  "Tim_McClelland_4"
Paddock_tested_db  =	"Clover"
#Zone_db            = 
data_file       = "Clover_Yld_SegID_Zone.csv"
...
annotate("text", x = 250, y= 1,label = "Only one zone")
...
#                             sheet = "2019 full data", range = cell_cols("A:O"))
 ...
 site <- filter(harm_database,
                Paddock_tested == "Clover_south_clay_loam") %>%
                #Paddock_tested == Paddock_tested_db) %>%  
                #Paddock_tested == Paddock_tested_db &
                #  Farmer == Farmer_db) %>% 
   dplyr::select(5, 6: 11)
