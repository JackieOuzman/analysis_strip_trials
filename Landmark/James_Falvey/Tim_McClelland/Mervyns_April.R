...
#file://Fssa2-adl/clw-share1/Microlab/value_soil_testing_prj/Yield_data/Landmark/James_Falvey_2/Tim_McClelland_4/Mervyns/output/Mervyns_Yld_SegID.csv

#As it appears in the database
Organisation_db	  = "Landmark"
Contact_db = "James_Falvey_2"
Farmer_db  =  "Tim_McClelland_4"
Paddock_tested_db  =	"Mervyns"
#Zone_db            = 
data_file       = "Mervyns_Yld_SegID.csv"
...
segments <- ggplot(seg_ID_t_test_summary, aes(SegmentID , Yld, group = Rate_as_factor))+
  ...
  labs(x= "Distance along the strip (meters)",
       y = "Yield t/ha",
       title = "",
       subtitle = paste0(Paddock_tested_db, ". Note: GSP is between 50 and 59, labelled as 50.59"),
       caption = "")#+
...
zone_av_1_rate3vsGR_res_sig
 ...
 write.csv(zone_av_1, paste0(graph_path,"/t_testzone_whole_strip_av.csv"))
 ...
 zone_av_2$Rate_as_factor  <- as.factor(zone_av_2$Rates)
 zone_2 <- ggplot( zone_av_1, aes(Rate_as_factor, Yld))+
...
mean_zone_av_p_values <- dplyr::select(mean_zone_av_p_values, Rates, Significant#,
                                       #Zone
                                       )
...
mean_zone_av_output_display <- left_join(mean_zone_av_output_display, 
...
# mean_zone_av_output_display <- spread(mean_zone_av_output_display, 
#                                       #Zone, 
#                                       Yld)
mean_zone_av_output_display
#remove the NA
