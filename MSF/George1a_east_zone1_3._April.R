...
#"\\FSSA2-ADL\clw-share1\Microlab\value_soil_testing_prj\Yield_data\MSF\Michael_Moodie_Todd_McDonald\Pole\Georges\output\Georges_Yld_SegID_Zone_East.csv"

#As it appears in the database
Organisation_db	  = "MSF"
Contact_db = "Michael_Moodie_Todd_McDonald"
Farmer_db  =  "Pole"
Paddock_tested_db  =	"Georges"
#Zone_db            = 
data_file       = "Georges_Yld_SegID_Zone_East.csv"
...
#seg_ID <- dplyr::filter(seg_ID, !between(seg_ID$SegmentID, 88, 89))
 seg_ID <- dplyr::filter(seg_ID, SegmentID != 200)
 seg_ID <- dplyr::filter(seg_ID, SegmentID != 190)
 seg_ID <- dplyr::filter(seg_ID, SegmentID != 220)
 seg_ID <- dplyr::filter(seg_ID, SegmentID != 230)
 seg_ID <- dplyr::filter(seg_ID, SegmentID != 210)
# seg_ID <- dplyr::filter(seg_ID, SegmentID != 850)
# seg_ID <- dplyr::filter(seg_ID, SegmentID != 940)
# seg_ID <- dplyr::filter(seg_ID, SegmentID != 910)
...
segments <- ggplot(seg_ID_t_test_summary, aes(SegmentID , Yld, group = Rate_as_factor))+
  ...
  labs(x= "Distance along the strip (meters)",
       y = "Yield t/ha",
       title = "",
       subtitle = paste0(Paddock_tested_db, " ", name),
       caption = "")+
...
annotate("text", x = 1000, y= 5,label = "Some missing data")
...
ggsave(path= graph_path, filename = paste0("t-test_segments_", name, ".png"), device = "png" ,
       width = 20, height = 10, units = "cm")

seg_ID_t_test_summary <- left_join(seg_ID_t_test_summary,list_rates)

write.csv(seg_ID_t_test_summary, paste0(graph_path,"/t_test_segments_", name, ".csv"))
...
zone_av_1_rate3vsGR_res_sig
 ...
 ggsave(path= graph_path, filename = paste0("t-test_zone_zone1_strip_", name, ".png"), device = "png" ,
        width = 20, height = 10, units = "cm")
 ...
 write.csv(zone_av_1, paste0(graph_path,"/t_testzone_zone1_av_", name, ".csv"))
 ...
 ##save the graphs of the zone strip work
 ggsave(path= graph_path, 
        filename = paste0("t-test_zone_zone2_strip_", name, ".png"), device = "png" ,
        width = 20, height = 10, units = "cm")
 ...
 mean_zone_av_2
 write.csv(zone_av_2, paste0(graph_path,"/t_testzone_zone2_av_", name, ".csv"))
 ...
 ##save the graphs of the zone strip work
 ggsave(path= graph_path, 
        paste0(filename = "t-test_zone_zone3_strip", name, ".png"), 
        device = "png" ,
        width = 20, height = 10, units = "cm")
 ...
 write.csv(zone_av_3, paste0(graph_path,"/t_testzone_zone3_av_", name, ".csv"))
...
harm_database <- read_excel(paste0(
 ...
 site <-site %>% 
   mutate(`Colwell rec rate` = replace(`Colwell rec rate`,
                                       ...
                                       "9999"),
          `DGT rec rate` = replace(`Colwell rec rate`,
                                       `Colwell rec rate` == 'Replacement', 
                                   "9999"))
...
ggsave(path= graph_path, 
       filename = paste0(paddock, "_collection_", name, ".png"), 
       device = "png", 
