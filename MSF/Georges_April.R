...
#file://Fssa2-adl/clw-share1/Microlab/value_soil_testing_prj/Yield_data/MSF/Michael_Moodie_Todd_McDonald/Hunt/Swamp/processing/Swamp_Yld_SegID_zones.csv
#As it appears in the database
Organisation_db	  = "MSF"
Contact_db = "Michael_Moodie_Todd_McDonald"
Farmer_db  =  "Hunt"
Paddock_tested_db  =	"Swamp"
#Zone_db            = 
data_file       = "Swamp_Yld_SegID_zones.csv"
...
seg_ID <- dplyr::filter(seg_ID, SegmentID != 1290)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 850)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 840)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 820)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 830)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 850)
...
zone_av_1_rate3vsGR_res_sig
 ...
 zone_1 <- ggplot( zone_av_1, aes(Rate_as_factor, Yld))+
   ...
   annotate("text", x = 2, y= 3, size = 3,label = "box plot = 25%, 50%, 75%, dashed line = mean")
 zone_1
