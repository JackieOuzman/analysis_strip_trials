...
#file://Fssa2-adl/clw-share1/Microlab/value_soil_testing_prj/Yield_data/MSF/Michael_Moodie_Todd_McDonald/Nulty/Kirbys/output/Kirbys_Yld_SegID_zone.csv

#As it appears in the database
Organisation_db	  = "MSF"
Contact_db = "Michael_Moodie_Todd_McDonald"
Farmer_db  =  "Nulty"
Paddock_tested_db  =	"Kirbys"
#Zone_db            = 
data_file       = "Kirbys_Yld_SegID_zone.csv"
...
# seg_ID <-
#   read_csv(
#     paste0(name_of_path, "/", "processing",  "/", data_file),
#     col_types = cols_only(
#       'col4' = col_integer(),
#       'col8' = col_character(),
#       'col12' = col_guess(),
...
#seg_ID <- dplyr::select(seg_ID,"Yld", "Rates", "Zone", "DistOnLine" )
str(seg_ID)
...
#No data in segments
seg_ID <- dplyr::filter(seg_ID, SegmentID != 220)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 260)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 270)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 280)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 530)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 630)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 640)
#sparse data
seg_ID <- dplyr::filter(seg_ID, SegmentID != 290)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 300)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 520)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 550)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 560)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 580)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 210)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 320)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 500)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 570)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 600)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 610)

seg_ID <- dplyr::filter(seg_ID, SegmentID != 510)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 540)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 590)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 620)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 470)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 480)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 330)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 350)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 460)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 490)

seg_ID <- dplyr::filter(seg_ID, SegmentID != 310)
seg_ID <- dplyr::filter(seg_ID, SegmentID != 340)
...
test <- seg_ID_rate2vsGR %>% group_by(SegmentID, Rates) %>% tally()
...
test <- seg_ID_rate3vsGR %>% group_by(SegmentID, Rates) %>% tally()
...
seg_ID_t_test_summary %>% group_by(Zone, Rates) %>% tally()
seg_ID_t_test_summary %>% group_by(Zone) %>% tally()
...
segments <- ggplot(seg_ID_t_test_summary, aes(SegmentID , Yld, group = Rate_as_factor))+
  ...
  labs(x= "Distance along the strip (meters)",
       y = "Yield t/ha",
       title = "",
       subtitle = paste0(Paddock_tested_db," Lots of missing data - 66 data pts in z3 sand, 6 in z2 nice, z1 has no data"),
       caption = "")+
   #annotate("rect", xmin = zone1_min, xmax = zone1_max, ymin = 0, ymax = 2, #Zone 1
    #       alpha = .2) +
 # annotate("text", x = zone1_range, y= 1,label = zone1)+
  
   annotate("rect", xmin =zone2_min , xmax = zone2_max, ymin = 0, ymax = 2, #zone 2
            alpha = .2)+
  annotate("text", x = zone2_range, y= 1,label = zone2)+
  
   annotate("rect", xmin =zone3_min , xmax = zone3_max, ymin = 0, ymax = 2, #zone 3
            alpha = .2)+
   annotate("text", x = zone3_range, y= 1,label = zone3)+
annotate("text", x = 100, y= 0,label = "Lots of missing data")
...
mean_zone_av_output <- as.data.frame(mean_zone_av_3)
...
mean_zone_av_p_values <- mean_zone_av_3 
