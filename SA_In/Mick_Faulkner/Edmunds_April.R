...
#file://Fssa2-adl/clw-share1/Microlab/value_soil_testing_prj/Yield_data/SA_Ind/Mick_Faulkner_4/Edmunds/Arnold/output/Arnold_Yld_SegID_Zones.csv

#As it appears in the database
Organisation_db	  = "SA_Ind"
Contact_db = "Mick_Faulkner_4"
Farmer_db  =  "Edmunds"
Paddock_tested_db  ="Arnold"	
Paddock_tested_db_1  =  ""
#Zone_db            = 
data_file       = "Arnold_Yld_SegID_Zones.csv"
...
segments <- ggplot(seg_ID_t_test_summary, aes(SegmentID , Yld, group = Rate_as_factor))+
  ...
  labs(x= "Distance along the strip (meters)",
       ...
       caption = "")+
   annotate("rect", xmin = zone1_min, xmax = zone1_max, ymin = 0, ymax = 10, #Zone 1
           alpha = .2) +
  annotate("text", x = zone1_range, y= 1,label = zone1)+
  
   annotate("rect", xmin =zone2_min , xmax = zone2_max, ymin = 0, ymax = 10, #zone 2
            alpha = .2)+
