...
#This need to be the rates for low medium and high
# the best would to have it in the scripts and then it should be in the merged file
# or could have a note in the done file.
...
# Comparision of yield response: The closest match to recommended fertiliser rates compared to higher and lower fertiliser rates.

The soil test results results in a recommeded N and P rate.

Sean has done these calulations.

The recommeded fertilier rate was not always implemented, so I have wored out the closest match.
...
If we need to choose what rates to incuded it is best to use this rough rule:

- Avoid 0 rates (won't always be practical)
- lower rate should be close to half the recommeded rate
- higher rate should be close to double the recommed rate.
...
Rec_rate_comparison_plot <-
  ggplot(Rec_rate_comparison, mapping = aes(x = yld_response, fill = Significant)) +
  geom_bar() +
  facet_grid(Strip_Type ~ comparison)+ 
  #labeller(Strip_Type = label_wrap_gen(width = 16 ),
  #         comparison = label_wrap_gen(width = 16))) +
  ...
  labs(
    title = "Comparison of yield response for apporx. recommed rate vs higher/lower rates",
    subtitle = "The yield response is classed as positive when higher rates of fertiliser result in higher yields",
    y = "Count of paddocks in yield response class",
    x = "yield response"
  )
...
### Approx recomomed rate vs higher fertiliser rates
#### The rates were used for the anlysis
...
### Approx recomomed rate vs lower fertiliser rates
#### The rates were used for the anlysis
...
3. Are the rates included in the comaprision analysis sensible and what you want.
