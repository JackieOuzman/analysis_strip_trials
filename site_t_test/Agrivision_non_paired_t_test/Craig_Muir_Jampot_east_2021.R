...
function_paired_ttest_rate_order <- function(strips, zone_x){
  ...
  zone_x_high_vs_low_av <- group_by(zone_x_high_vs_low, SegmentID, Rate, Zone, rate_name, zone_name ) %>% 
    summarise_all(mean, na.rm= TRUE)
  #ensure that the dataset is duplictaed
  list_SegmentID_values <- zone_x_high_vs_low_av$SegmentID[duplicated(zone_x_high_vs_low_av$SegmentID)] #this returns a list of values I want to keep
  zone_x_high_vs_low_av <- zone_x_high_vs_low_av %>% filter(SegmentID %in% list_SegmentID_values)
  # run paired ttest
  zone_x_high_vs_low_res <- t.test(YldMassDry ~ Rate, data = zone_x_high_vs_low_av, paired = TRUE)
  ...
  #average the yld per segment and rate
  zone_x_high_vs_medium_av <- group_by(zone_x_high_vs_medium, SegmentID, Rate, Zone, rate_name, zone_name ) %>% 
    summarise_all(mean, na.rm= TRUE)
  #ensure that the dataset is duplictaed
  list_SegmentID_values <- zone_x_high_vs_medium_av$SegmentID[duplicated(zone_x_high_vs_medium_av$SegmentID)] #this returns a list of values I want to keep
  zone_x_high_vs_medium_av <- zone_x_high_vs_medium_av %>% filter(SegmentID %in% list_SegmentID_values)
  # run paired ttest
  zone_x_high_vs_medium_res <- t.test(YldMassDry ~ Rate, data = zone_x_high_vs_medium_av, paired = TRUE)
  ...
  #average the yld per segment and rate
  zone_x_medium_vs_low_av <- group_by(zone_x_medium_vs_low, SegmentID, Rate, Zone, rate_name, zone_name ) %>% 
    summarise_all(mean, na.rm= TRUE)
  #ensure that the dataset is duplictaed
  list_SegmentID_values <- zone_x_medium_vs_low_av$SegmentID[duplicated(zone_x_medium_vs_low_av$SegmentID)] #this returns a list of values I want to keep
  zone_x_medium_vs_low_av <- zone_x_medium_vs_low_av %>% filter(SegmentID %in% list_SegmentID_values)
  # run paired ttest
  zone_x_medium_vs_low_res <- t.test(YldMassDry ~ Rate, data = zone_x_medium_vs_low_av, paired = TRUE)
...
function_paired_ttest_GR_low_high <- function(GR_vs_low_High_rate, zone_x){
  ...
  #average the yld per segment and rate
  zone_x_GSP_vs_low_av <- group_by(zone_x_GSP_vs_low, SegmentID, Rate, Zone, rate_name, zone_name ) %>% 
    summarise_all(mean, na.rm= TRUE)
  #ensure that the dataset is duplictaed
  list_SegmentID_values <- zone_x_GSP_vs_low_av$SegmentID[duplicated(zone_x_GSP_vs_low_av$SegmentID)] #this returns a list of values I want to keep
  zone_x_GSP_vs_low_av <- zone_x_GSP_vs_low_av %>% filter(SegmentID %in% list_SegmentID_values)
  # run paired ttest
  zone_x_GSP_vs_low_res <- t.test(YldMassDry ~ Rate, data = zone_x_GSP_vs_low_av, paired = TRUE)
  ...
  #average the yld per segment and rate
  zone_x_GSP_vs_high_av <- group_by(zone_x_GSP_vs_high, SegmentID, Rate, Zone, rate_name, zone_name ) %>% 
    summarise_all(mean, na.rm= TRUE)
  #ensure that the dataset is duplictaed
  list_SegmentID_values <- zone_x_GSP_vs_high_av$SegmentID[duplicated(zone_x_GSP_vs_high_av$SegmentID)] #this returns a list of values I want to keep
  zone_x_GSP_vs_high_av <- zone_x_GSP_vs_high_av %>% filter(SegmentID %in% list_SegmentID_values)
  # run paired ttest
  zone_x_GSP_vs_high_res <- t.test(YldMassDry ~ Rate, data = zone_x_GSP_vs_high_av, paired = TRUE)
