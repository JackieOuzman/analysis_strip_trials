...
function_paired_ttest_rec_rate_low_high <- function(recom_rate1, zone_x, comp){
  ...
  zone_x_rec_r_p_vs_x_av <- group_by(zone_x_rec_r_p_vs_x, SegmentID, Rate, Zone, rate_name, zone_name ) %>% 
    summarise_all(mean, na.rm= TRUE)
  #ensure that the dataset is duplictaed
  list_SegmentID_values <- zone_x_rec_r_p_vs_x_av$SegmentID[duplicated(zone_x_rec_r_p_vs_x_av$SegmentID)] #this returns a list of values I want to keep
  zone_x_rec_r_p_vs_x_av <- zone_x_rec_r_p_vs_x_av %>% filter(SegmentID %in% list_SegmentID_values)
  # run paired ttest
  zone_x_rec_r_p_vs_x_res <- t.test(YldMassDry ~ Rate, data = zone_x_rec_r_p_vs_x_av, paired = TRUE)
  
  #####test results
  # Report values from the t.test
  zone_x_rec_rate_p_vs_x_res_sig <-
    data.frame(P_value = as.double(zone_x_rec_r_p_vs_x_res$p.value),
               Mean_diff = (zone_x_rec_r_p_vs_x_res$estimate)) %>%
    mutate(
