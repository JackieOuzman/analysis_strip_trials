...
input_file <-"Aldermans_Yld_SegID_Zone.csv"
name_Paddock <- unlist(strsplit(input_file,"_"))[1]
## add this into the strips df
name_Paddock <- "Lister Trading - Aldermans"
name_Paddock
...
for_plotting <- for_plotting %>% 
  mutate(rate_as_factor = case_when(
    rate_as_factor == "100" ~   "46 / strip av 5.85",
    rate_as_factor == "200" ~  "92 / strip av 6.21",
    rate_as_factor == "400" ~  "184 / strip av 5.74",
    
  ))
# oder rate_as_factor

for_plotting$rate_as_factor <-  factor(for_plotting$rate_as_factor, 
                                       labels=c("46 / strip av 5.85", 
                                                "92 / strip av 6.21",
                                                "184 / strip av 5.74"))
...
collection1
ggsave( filename = 
          "W:/value_soil_testing_prj/Yield_data/2020/processing/r_outputs/ws_plots_etc/Claire_Aldermans_N.png",
