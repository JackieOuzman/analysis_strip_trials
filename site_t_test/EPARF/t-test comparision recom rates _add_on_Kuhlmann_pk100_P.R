...
## !!! reclass one of the lables for zone 1 i have 2 'rate3'
recom_rate1_summary_zone1 <- recom_rate1_summary_zone1 %>% 
  mutate(approx_p_rec = case_when(
    Rate == 0.0 ~ "rate2",
    TRUE ~ approx_p_rec
  ))
...
zone_1_filter <- recom_rate1 %>% 
  filter(Rate %in% c(2.5,5,10) & zone_name == "zone1") #what is in the bracket we will keep
