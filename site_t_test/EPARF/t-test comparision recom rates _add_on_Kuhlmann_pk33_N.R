...
#View(fert_app_all_steps)
## !!! I need to remove the P trial info here and only keep the N

fert_app_all_steps <- filter(fert_app_all_steps,
                             Strip_Type == "N Strip") 
...
recom_rate1 <- recom_rate1 %>% 
  mutate(
    ...
    comparison_rec_rate_low_n = case_when(
      rec_rate_high_low_n == "rec_rate_n"         ~       "rec_rate_low_n",
...
#View(rec_rate_n_vs_low_High_wide)
#####
...
#what ran?
rec_rate_n_vs_lower_zone_1 # run#
rec_rate_n_vs_lower_zone_2 # not run

rec_rate_n_vs_higher_zone_1#not
rec_rate_n_vs_higher_zone_2# run
