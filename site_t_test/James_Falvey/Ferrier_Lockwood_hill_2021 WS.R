...
input_file <-"LockwoodHill_Yld_SegID_Zone.csv"
name_Paddock <- unlist(strsplit(input_file,"_"))[1]
## add this into the strips df
name_Paddock <- "Ferrier (Wirrabilla) - Lockwood hill"
name_Paddock
...
function_strip_plot <- function(for_plotting){
  ...
  for_plotting <- for_plotting %>% 
    mutate(rate_as_factor = case_when(
      rate_as_factor == "40" ~   "18 / strip av 3.48",
      rate_as_factor == "80" ~   "37 / strip av 3.53",
      rate_as_factor == "160" ~  "74 / strip av 3.40"
      
    ))
  # oder rate_as_factor
  
  for_plotting$rate_as_factor <-  factor(for_plotting$rate_as_factor, 
                                         labels=c("18 / strip av 3.48", 
                                                  "37 / strip av 3.53",
                                                  "74 / strip av 3.40"
                                         ))
...
collection1
ggsave( filename = 
          "W:/value_soil_testing_prj/Yield_data/2020/processing/r_outputs/ws_plots_etc/Lehaman_Lockwood_Hill_N.png",
