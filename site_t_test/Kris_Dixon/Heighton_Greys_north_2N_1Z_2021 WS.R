...
Trial_type <- "N"  # "N" "P"
Number_of_strips <-   "strip2" # "strip2" "strip3"
...
input_file <-"GreysNorth_Yld_SegID_zone.csv"
name_Paddock <- unlist(strsplit(input_file,"_"))[1]
## add this into the strips df
name_Paddock <- "Heighton - Greys north"
name_Paddock
...
Rates_labels <- data.frame(Rate = unique(strips$Rate)) %>% 
  ...
  mutate(rate_name_order = c("low", "high"))
...
function_strip_plot <- function(for_plotting){
  ...
  for_plotting <- for_plotting %>% 
    mutate(rate_as_factor = case_when(
      rate_as_factor == "60" ~    "28 / strip av 4.68",
      rate_as_factor == "120" ~   "55 / strip av 4.58"
      
    ))
  # oder rate_as_factor
  
  for_plotting$rate_as_factor <-  factor(for_plotting$rate_as_factor, 
                                         labels=c("28 / strip av 4.68", 
                                                  "55 / strip av 4.58"
                                         ))
...
collection1
ggsave( filename = 
          "W:/value_soil_testing_prj/Yield_data/2020/processing/r_outputs/ws_plots_etc/Kris_Dixon_Heighton_GreyNth_N.png",
...
for_ricks_tables_summary <- for_ricks_tables_summary %>% 
  ...
  tidyr::pivot_longer(cols = c("yld_resposne_high_v_low"),   
            names_to = "comparison",
               values_to = "yld_response") %>% 
  # dplyr::select(Zone_ID, comparison, yld_response, low, medium, high,
  dplyr::select(Zone_ID, comparison, yld_response, low,  high,
...
function_paired_ttest_rate_order <- function(strips, zone_x){
  ...
  zone_x_high_vs_medium_vs_low_res_sig <- zone_x_high_vs_low_res_sig
...
## filter out one rate so we only have 3
 # GR_vs_low_High_rate <- GR_vs_low_High_rate %>%
 #   filter(Rate == c(140,240)) # this is what we will keep 2.5,17.9
...
str(GR_vs_low_High_rate)
#!!! waring below function has been modified
