...
input_file <-"Channel_Yld_SegID_Zone.csv"
name_Paddock <- unlist(strsplit(input_file,"_"))[1]
## add this into the strips df
name_Paddock <- "Marall holdings - Channel"
name_Paddock
...
function_strip_plot <- function(for_plotting){
  ...
  for_plotting <- for_plotting %>% 
    mutate(rate_as_factor = case_when(
      rate_as_factor == "60" ~   "13 / strip av 3.83",
      rate_as_factor == "100" ~  "22 / strip av 3.96"
      
    ))
  # oder rate_as_factor
  
  for_plotting$rate_as_factor <-  factor(for_plotting$rate_as_factor, 
                                         labels=c("13 / strip av 3.83", 
                                                  "22 / strip av 3.96"
                                                  ))
...
ggsave( filename = 
          "W:/value_soil_testing_prj/Yield_data/2020/processing/r_outputs/ws_plots_etc/Kris_Dixon_Marshall_holding_P.png",
...
grand_mean_std_error_high_low
grand_mean_std_error_high_medium
grand_mean_std_error_medium_low
...
grand_mean_se <-grand_mean_std_error_high_low
