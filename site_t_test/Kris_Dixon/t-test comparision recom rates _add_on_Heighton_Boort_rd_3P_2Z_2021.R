...
zone_1_filter <- recom_rate1 %>% 
  filter(Rate %in% c(30,60) & zone_name == "zone1") #what is in the bracket we will keep
zone_2_filter <- recom_rate1 %>% 
  filter(Rate %in% c(30,60) & zone_name == "zone2")
...
function_paired_ttest_rec_rate_low_high <- function(recom_rate1, zone_x, comp){
  ...
  list_SegmentID_values_rec_rate_l <- zone_x_rec_r_p_vs_x_av$SegmentID[duplicated(zone_x_rec_r_p_vs_x_av$SegmentID)] #this returns a list of values I want to keep
...
#what ran?
rec_rate_p_vs_lower_zone_1 # no
rec_rate_p_vs_lower_zone_2 # no

rec_rate_p_vs_higher_zone_1 # yes
