...
str(recom_rate1)
unique(recom_rate1$Rate)
fert_app_all_steps %>%  filter(Paddock_ID == 52472) %>% 
distinct(Rate)
...
##???? recom rates problem here
names(fert_app_all_steps)
names(recom_rate1)
#Subset fert_app_all_steps


recom_rate1 <- left_join(recom_rate1, fert_app_all_steps)
recom_rate1 %>%  filter(Paddock_ID == 52472) %>% 
  distinct(Rate)
str(recom_rate1)
#View(recom_rate1)
###############################################################################################################
...
zone_1_filter <- recom_rate1 %>% 
  filter(Rate %in% c(88.2, 42.2) & zone_name == "zone1") #what is in the bracket we will keep
zone_2_filter <- recom_rate1 %>% 
  filter(Rate %in% c(88.2, 42.2) & zone_name == "zone2")
zone_3_filter <- recom_rate1 %>% 
  filter(Rate %in% c(88.2, 42.2) & zone_name == "zone3")
