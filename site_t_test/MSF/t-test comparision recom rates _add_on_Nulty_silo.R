...
zone_1_filter <- recom_rate1 %>% 
  filter(Rate %in% c(70,120) & zone_name == "zone1") #what is in the bracket we will keep
zone_2_filter <- recom_rate1 %>% 
  filter(Rate %in% c(70,120) & zone_name == "zone2")
zone_3_filter <- recom_rate1 %>% 
  filter(Rate %in% c(0,35) & zone_name == "zone3")
...
rec_rate_n_vs_higher_zone_3 # yes
