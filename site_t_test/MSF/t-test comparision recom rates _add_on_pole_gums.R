...
#!! changed the code here beacsue the paddock Id is much longer than the other 6 replaces 5
...
#!! changed the code here beacsue we have an extra strip that we wont analysis??
unique(fert_app_all_steps$Total_sum_N_content)
fert_app_all_steps <- fert_app_all_steps %>% 
  dplyr::filter(Total_sum_N_content != 0.00)
    ...
    approx_n_rec =
      dplyr::case_when(
        ...
        difference_n == max(recom_rate1_summary_zone1$difference_n[recom_rate1_summary_zone1$difference_n !=
                                                                     max(recom_rate1_summary_zone1$difference_n)]) ~ "rate3",
        #use this if you have 4 rates best match rate 1 -3
        difference_n == max(recom_rate1_summary_zone1$difference_n) ~ "rate4",
        #difference_n == max(recom_rate1_summary_zone1$difference_n) ~ "rate2",
        TRUE ~ as.character("rate2")
      )
  )
    ...
    approx_n_rec =
      dplyr::case_when(
        ...
        difference_n == max(recom_rate1_summary_zone2$difference_n[recom_rate1_summary_zone2$difference_n !=
                                                                     max(recom_rate1_summary_zone2$difference_n)]) ~ "rate3",
        #use this if you have 4 rates best match rate 1 -3
        difference_n == max(recom_rate1_summary_zone2$difference_n) ~ "rate4",
    ...
    approx_n_rec =
      dplyr::case_when(
        ...
        difference_n == max(recom_rate1_summary_zone3$difference_n[recom_rate1_summary_zone3$difference_n !=
                                                                     max(recom_rate1_summary_zone3$difference_n)]) ~ "rate3",
        #use this if you have 4 rates best match rate 1 -3
        difference_n == max(recom_rate1_summary_zone3$difference_n) ~ "rate4",
    ...
    approx_n_rec =
      dplyr::case_when(
        ...
        difference_n == max(recom_rate1_summary_zone4$difference_n[recom_rate1_summary_zone4$difference_n !=
                                                                     max(recom_rate1_summary_zone4$difference_n)]) ~ "rate3",
        #use this if you have 4 rates best match rate 1 -3
        difference_n == max(recom_rate1_summary_zone4$difference_n) ~ "rate4",
...
zone_1_filter <- recom_rate1 %>% 
  filter(Rate %in% c(40.78,80.80) & zone_name == "zone1") #what is in the bracket we will keep
zone_2_filter <- recom_rate1 %>% 
  filter(Rate %in% c(3.30,21.00,40.78) & zone_name == "zone2")
zone_3_filter <- recom_rate1 %>% 
  filter(Rate %in% c(40.78,80.80) & zone_name == "zone3")
zone_4_filter <- recom_rate1 %>% 
  filter(Rate %in% c(3.30,21.00) & zone_name == "zone4")

unique(zone_1_filter$Rate)
...
# ## higher than N/P rate comparision
...
function_paired_ttest_rec_rate_low_high <- function(recom_rate1, zone_x, comp){
  ...
  list_SegmentID_values_rec_rate_l <- zone_x_rec_r_n_vs_x_av$SegmentID[duplicated(zone_x_rec_r_n_vs_x$SegmentID)] #this returns a list of values I want to keep
