...
GR_vs_low_High_rate_summary <- GR_vs_low_High_rate_wide %>%
  mutate(
    ...
    yld_resposne_GSP_v_low =  case_when(
     ...
     #   GSP_vs_higher  < 0 +  se_comp_GSP_high ~ "negative", #this is changed from > to <
     #   GSP_vs_higher  > 0 -  se_comp_GSP_high ~ "positive", #this is changed from < to >
     #   TRUE ~ "no_response"
    )
