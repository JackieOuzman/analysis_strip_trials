...
input_file <-"Stefan_Slater_Yld_Seg_ID_zone.csv"
name_Paddock <- unlist(strsplit(input_file,"_"))[1]
## add this into the strips df
name_Paddock <- "Stefan Slater"
name_Paddock
...
for_plotting <- for_plotting %>% 
  mutate(rate_as_factor = case_when(
    rate_as_factor == "0" ~ "0 / strip av 3.83",
    rate_as_factor == "105" ~ "21 / strip av 3.81",
    rate_as_factor == "180" ~ "36 / strip av 4.13"
    
  ))
# oder rate_as_factor

for_plotting$rate_as_factor <-  factor(for_plotting$rate_as_factor, 
                                       labels=c("0 / strip av 3.83", 
                                                "21 / strip av 3.81",
                                                "36 / strip av 4.13"))
...
collection
ggsave( filename = 
          "W:/value_soil_testing_prj/Yield_data/2020/processing/r_outputs/ws_plots_etc/Schmitt_Slater_P.png",
