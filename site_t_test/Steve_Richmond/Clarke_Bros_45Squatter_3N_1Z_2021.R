...
input_file <-"45Squatters_Yld_SegID_zones.csv"
name_Paddock <- unlist(strsplit(input_file,"_"))[1]
## add this into the strips df
name_Paddock <- "Clarke Bros - 45 SQUATTERS"
name_Paddock
...
function_zone_plots_empty <- function(for_plotting, zone_x){
...
assign(("plot_zone2"), function_zone_plots_empty(for_plotting, 2))
############################################################################################################
...
## filter out one rate so we only have 3
 GR_vs_low_High_rate <- GR_vs_low_High_rate %>%
   filter(Rate == c(20,40)) # this is what we will keep 2.5,17.9
