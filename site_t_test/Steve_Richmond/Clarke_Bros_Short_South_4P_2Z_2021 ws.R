...
input_file <-"ShortSouth_Yld_SegID_Zones.csv"
name_Paddock <- unlist(strsplit(input_file,"_"))[1]
## add this into the strips df
name_Paddock <- "Clarke Bros - Short South"
name_Paddock
...
for_plotting <- for_plotting %>% 
  mutate(rate_as_factor = case_when(
    rate_as_factor == "0" ~   "0 / strip av 4.15",
    rate_as_factor == "30" ~  "7 / strip av 4.42",
    rate_as_factor == "60" ~  "13 / strip av 4.51",
    rate_as_factor == "120" ~ "26 / strip av 4.83"
    
  ))
# oder rate_as_factor

for_plotting$rate_as_factor <-  factor(for_plotting$rate_as_factor, 
                                       labels=c("0 / strip av 4.15", 
                                                "7 / strip av 4.42",
                                                "13 / strip av 4.51",
                                                "26 / strip av 4.83"))
...
collection
ggsave( filename = 
          "W:/value_soil_testing_prj/Yield_data/2020/processing/r_outputs/ws_plots_etc/Clarke_Bros_Short_sth_P.png",
