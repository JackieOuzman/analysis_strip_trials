...
input_file <-"Long_P_Yld_SegID_Zone.csv"
name_Paddock <- unlist(strsplit(input_file,"_"))[1]
## add this into the strips df
name_Paddock <- "Prime Family Trust - Long (P)"
name_Paddock
...
for_plotting <- for_plotting %>% 
  mutate(rate_as_factor = case_when(
    rate_as_factor == "0" ~ "0 / strip av 2.47",
    rate_as_factor == "60" ~ "60 / strip av 2.31",
    rate_as_factor == "120" ~ "120 / strip av 2.31"
  ))
...
collection
ggsave( filename = 
         "W:/value_soil_testing_prj/Yield_data/2020/processing/r_outputs/ws_plots_etc/prime_faily_trust_longP.png",
