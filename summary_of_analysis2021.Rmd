...
plot1 <-
  dplyr::distinct(spatial_data_no_yld_df, Paddock_ID , .keep_all = TRUE) %>%
...
#Note the below plot run off an step1 script been run and succesful output
#"W:\value_soil_testing_prj\Yield_data\2020\processing\r_scripts\step1_spatial_data_no_yld.R"

spatial_data_no_yld <- read.csv("W:/value_soil_testing_prj/Yield_data/2020/processing/processing_files/step1_spatial_data_no_yld_2021-02-18.csv")
...
- low rates of fertiliser applied (eg too litlle to see a strong yield response).

However despite these wide range of reasons it was thought that yield varaition is was driving these observations.
...
We have compared the GSP and Alt GSP per zone.
t test and mean values
I dont have the example paddocks run yet but its looking ok, when this is done I need to merge into one file
 
#"W:\value_soil_testing_prj\Yield_data\2020\processing\r_outputs\GSP"
...
GSPvsAltGSP_plot <-ggplot(GR_AltGSP, mapping = aes(x=Significant ))+
  geom_bar() +
  facet_grid(.~ Strip_Type)+
  theme_bw()+
  geom_text(aes(y = ((..count..)), label = ((..count..))), stat = "count", vjust = -0.25) +
  labs(title = "Comparison of yield response in strips with the same fertiliser rate to reprseent the GSP (per zone)", 
       subtitle = "",
