...
I don't have the example paddocks run yet but its looking OK, when this is done I need to merge into one file
...
GSPvsAltGSP_plot <-ggplot(GSP_AltGSP_t_test, mapping = aes(x=Significant ))+
  geom_bar() +
  facet_grid(.~ Strip_Type)+
  theme_bw()+
  geom_text(aes(y = ((..count..)), label = ((..count..))), stat = "count", vjust = -0.25) +
  labs(title = "Comparison of yield response in strips with the same fertiliser rate to represent the GSP (per zone)", 
       subtitle = "",
...
write.csv(spatial_data_no_yld, "W:/value_soil_testing_prj/Yield_data/2020/processing/processing_files/step1_spatial_data_no_yld_2021-02-18_forSean.csv")
...
GR_comparison_plot <-
  ...
  geom_text(aes(y = ((..count..)), label = ((..count..))), stat = "count", vjust = -0.25) +
  labs(
    title = "Comparison of yield response in strips the GSP vs higher and lower rates",
    subtitle = "The yield response is classed as positive when higher rates of fertiliser result in higher yields",
    y = "Count of paddocks in yield response class",
    x = "yield response"
  )
